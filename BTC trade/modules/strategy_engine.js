...
export class StrategyEngine {
    ...
    generateSignal(ohlcv, currentPosition, averageEntry, fearGreed = null) {
        ...
        // ══════════════════════════════════════════════════════════════════
        // IMPROVED FILTERS — added based on live trade analysis
        // Root cause of losses: buying into bear markets and on weak signals
        // ══════════════════════════════════════════════════════════════════

        // HARD VETO #1 — Never BUY when price is below EMA200.
        // EMA200 = the definitive line between bull and bear market.
        // Buying below EMA200 means trading against the macro trend.
        // This is the single most impactful filter — eliminates bear market entries.
        if (currentPrice < ema200) {
            buyScore = 0;
            buyReasons.length = 0;
            buyReasons.push(`🚫 VETO: Price $${currentPrice.toFixed(0)} below EMA200 $${ema200.toFixed(0)} — bear market, no BUY allowed`);
        }

        // HARD VETO #2 — Never BUY when 4H trend is bearish AND MACD is bearish.
        // Prevents catching falling knives during confirmed multi-timeframe downtrends.
        if (h4BearTrend && macdBearish && currentPrice < ema200) {
            buyScore = 0;
        }

        // PENALTY — Bear trend alignment hurts BUY quality significantly
        if (bearTrend) {
            buyScore = Math.max(0, buyScore - 3);
            buyReasons.push(`⚠️ Bear trend active (EMA20<50<200) — reduced conviction`);
        }

        // PENALTY — MACD bearish in a trending market penalises BUY
        if (macdBearish && isTrending && !bullTrend) {
            buyScore = Math.max(0, buyScore - 2);
        }

        // PENALTY — 4H bearish trend penalises 1H BUY signals
        if (h4BearTrend) {
            buyScore = Math.max(0, buyScore - 2);
            buyReasons.push(`⚠️ 4H trend bearish — lower timeframe BUY has lower success rate`);
        }

        // BONUS — Require at least 2 independent momentum confirmations for full BUY.
        // Prevents entries on trend alone without momentum backing.
        const momentumCount = [macdBullish, rsiOversold || rsi < 45, stochOversold, mfiOversold, bullishPattern].filter(Boolean).length;
        if (momentumCount >= 2) {
            buyScore++;
            buyReasons.push(`Multi-momentum confirmed (${momentumCount}/5 signals)`);
        }

        // BONUS — ADX > 30 in bull trend = very strong trend, high-quality entry
        if (adx > 30 && bullTrend) {
            buyScore++;
